...
export type ProgramPlan = Static<typeof ProgramPlanSchema>;

export interface ProgramPlanValidation {
	success: boolean;
	errors: string[];
...
function schemaErrors(value: unknown): string[] {
	return [...Errors(ProgramPlanSchema, value)].map(
...
export function validateProgramPlan(value: unknown): ProgramPlanValidation {
	...
	const plan = value as ProgramPlan;
	const errors: string[] = [];
