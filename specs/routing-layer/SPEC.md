...
Queued steer/follow-up input is an unconditional continuation of an active running lease; it is evaluated before pending
hard boundaries and does not enter the ordinary interactive gate below. That rule is about delivery, not origin: pi sets
`streamingBehavior` only while the agent is streaming, for every source, so queued extension input is covered and idle
extension-generated input is not. Input generated by an extension's `sendUserMessage` while the agent is idle is
ordinary new user intent — it starts its own turn, is subject to hard boundaries and the gate below, and invalidates a
standing execution authorization. The router's own continuations are custom context messages that never emit an input
event, so they retain their lease without a source exemption. At any other user turn, combine deterministic intent/state
signals with expected cache value; escalate to a secondary "continuity" classification only when those signals are
inconclusive. Significant reusable cache should resist a marginal switch; very strong semantic discontinuity can still
override it. The significance test is `hasSignificantReusableCache` in
