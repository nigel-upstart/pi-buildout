...
- `/route off` takes effect immediately within an active session and bypasses all adapter routing and blocking behavior:
  ...
  collection, or router telemetry. Routing work already in flight may run to completion, but its result is discarded
  rather than applied, including when routing is re-enabled before it finishes. The dormant lease may be reused only
  after routing is explicitly re-enabled.
